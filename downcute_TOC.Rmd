...
[![](Images/(MA) Final Georeferenced Map.tif)](){target="_blank"}


# Access to transit in San Francisco
...
The following map continues to build on the previous one by merging adjacent grid cells that have the same value to create polygons. This helps us group larger areas within the city based on ease of accessibility, which is helpful when one is planning at a zoomed-out scale. As we can see from the nuances of the grids, there aren't large parts of the city where adjacent areas have the exact same accessibility scores. 

[![](Images/Accessibility Score (Polygons).png)](){target="_blank"}


# Mapping Shootings and Police Stations in Chicago
The following two maps analyse issues of shootings and police stations in Chicago.
...
# Multiracial Population in Maui
* Skill: Displaying data on an interactive map
* Skill: Aggregating point data to a layer of polygons

On clicking the image below, you would be directed to an interactive map. The map displays the percentage of population identifying with more than one race in different areas of Maui. Hovering over the specific polygons will provide the reader with information about the Census block, while clicking it will allow them to read the total population and percentage of multiracial population in that block. 
